import sys
sys.path.append('../..')
from UDAGCN_gc import UDAGCN_GC
import torch
...
parser = argparse.ArgumentParser(description='UDAGCN for Cross-Domain Recommendation')
parser.add_argument("--UDAGCN", type=int,default=0)
...
parser.add_argument('--domain', type=float, default=1., help='domain loss weight')
...
if __name__ == '__main__':
    ...
    avg_acc_list = []
    coeff = {"domain":args.domain}
    for r in range(args.repeat):
        best_acc = 0.

        model = UDAGCN_GC(args.UDAGCN, encoder_dim=encoder_dim,num_gcn_layers=num_gcn_layers,hidden_dims=hidden_dims,num_classes=num_classes, device=args.device, coeff=coeff).to(args.device)
        ...
        for epoch in range(1, args.epochs):
            ...
            if epoch%1==0:
                ...
                with open(logfile, 'a+') as f:
                    f.write(f'Epoch: {epoch:03d}, loss:{epoch_loss}, clf_loss:{epoch_clf_loss}, domain_loss；{epoch_d_loss}，Source Acc: {src_acc:.4f}, Target Acc: {tgt_acc:.4f}, best acc:{best_acc}\n')
